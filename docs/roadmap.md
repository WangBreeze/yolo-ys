...
更新日期：2026-09-13。入口：[开发 Wiki](wiki/README.md) · [架构](architecture.md) · [验证记录](validation.md)。
...
| R1 视频入库与语义理解 | 已实现，单个真实教程已检查 | you-get 原流下载、默认分类、语音和画面分析、报告与缓存 | 扩充教程样本，记录摘要正确性、关键步骤覆盖和耗时；修正把讲解推断为已完成任务的问题 |
...
4. **D04：Windows 原生检查。** 配置实际窗口、依赖、GPU、缩放和键位；核对客户区截图，完成 dry-run 和有限输入检查，保存实机证据。双平台 CI 配置存在，但远端运行结果尚未验证。
...
| 证据 | 结果与限制 | 查看位置 |
|---|---|---|
| Linux 回归 | 68 项测试、demo、动作经验 train、learned-demo 通过 | [验证记录](validation.md) |
