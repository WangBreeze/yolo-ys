...
更新日期：2026-09-20。Wiki 保存在仓库的 `docs/wiki/`，随源代码提交和迁移；本文是统一导航，具体实现说明继续维护在原文档中。

项目当前采用 **Linux 制作视频训练数据与候选命令、Windows 运行游戏** 的分工。Linux 已完成 20 集教程理解和 674 条候选命令生成，Windows 接口已实现但缺实机验收；当前尚无正式游戏检测模型或已验证实机策略。
