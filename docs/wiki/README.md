...
更新日期：2026-09-13。Wiki 保存在仓库的 `docs/wiki/`，随源代码提交和迁移；本文是统一导航，具体实现说明继续维护在原文档中。

项目当前采用 **Linux 制作视频训练数据、Windows 运行游戏** 的分工。Linux 离线流程已验证，Windows 接口已实现但缺实机验收；当前原神教程已有 32 个待标注帧，尚无正式游戏检测模型或已验证实机策略。
